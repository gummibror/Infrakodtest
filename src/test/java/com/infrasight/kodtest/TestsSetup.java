...
public abstract class TestsSetup {
	...
	static HttpClientWrapper httpClient;
		...
		for (int c = 0; c < 19; c++) {
			...
			try {
				...
				if (con.getResponseCode() == 200 || con.getResponseCode() == 401) {
					...
					// My api-services for the tests, objects and such
					String baseUrl = "http://localhost:"+ TestVariables.API_PORT;
					String authUrl = baseUrl+"/api/auth";
					httpClient = new HttpClientWrapper();
					authService = new AuthenticationService(httpClient);
					String authToken = authService.authenticate(authUrl, TestVariables.API_USER, TestVariables.API_PASSWORD);
					accountService = new AccountService(httpClient, baseUrl, authToken);
					groupService = new GroupService(httpClient, baseUrl, authToken);
